...
本文件基于原始 OpenAI CLIP 的整体骨架重写，并按照论文中的 SR-CLIP 结构进行扩展。

相对于原始 CLIP 的核心改动：
1. VisionTransformer 不再只输出单个全局图像特征，而是同时输出：
   - scene token（全局场景特征）
   - patch/object tokens（局部视觉特征）
2. 文本编码中引入可学习上下文向量（learnable context）。
3. 新增 Text_Decoder：
   - Text-to-Text
   - Visual-to-Text
4. 新增 VisualDecoder：
   - Visual-to-Visual
   - Text-to-Visual
5. 不再输出图文对比 logits，而是输出下游分割模型所需的：
   - TPrompt
   - VPrompt
   - text_token
   - image_token
...
class ResidualAttentionBlock(nn.Module):
    # [相对原始 CLIP 的修改]
    # 原始 CLIP 使用的是 fp16-safe LayerNorm；
    # 这里直接用 nn.LayerNorm，便于统一实现。
...
# =========================================================
# SR-CLIP / SAIST 新增：自定义注意力模块
# =========================================================

class Attention(nn.Module):
    # [SR-CLIP / SAIST 新增]
    # 原始 CLIP 直接使用 nn.MultiheadAttention；
    # 这里单独封装一个通用 Attention，方便构造：
    # 1) Text-to-Text
    # 2) Visual-to-Text
    # 3) Visual-to-Visual
    # 4) Text-to-Visual
...
# =========================================================
# SR-CLIP / SAIST 新增：四个基础交互子模块
# =========================================================

class TextToText(nn.Module):
    # [SR-CLIP / SAIST 新增]
    # 文本内部自注意力：对应论文中 Text Decoder 的第一步 Text-to-Text
...
class VisualToText(nn.Module):
    # [SR-CLIP / SAIST 新增]
    # 视觉引导文本：对应论文中 Text Decoder 的第二步 Visual-to-Text
    def __init__(self, dim, nhead, dropout=0.1):
        super().__init__()
...
class VisualToVisual(nn.Module):
    # [SR-CLIP / SAIST 新增]
    # 视觉内部自注意力：对应论文中 Visual Decoder 的第一步 Visual-to-Visual
    def __init__(self, dim, nhead, dropout=0.1):
        super().__init__()
        self.self_attn = Attention(dim, nhead, proj_drop=dropout)
        self.norm1 = nn.LayerNorm(dim)
...
class TextToVisual(nn.Module):
    # [SR-CLIP / SAIST 新增]
    # 文本引导视觉：对应论文中 Visual Decoder 的第二步 Text-to-Visual
    def __init__(self, dim, nhead, dropout=0.1):
        super().__init__()
        self.cross_attn = Attention(dim, nhead, proj_drop=dropout)
        self.norm1 = nn.LayerNorm(dim)
...
class TokenExpanderMLP(nn.Module):
    def __init__(self, dim: int, num_tokens: int, hidden_dim: Optional[int] = None):
        ...
        self.mlp = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(hidden_dim, num_tokens * dim),
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.dim() == 3:
            if x.shape[1] != 1:
                raise ValueError(
                    f"TokenExpanderMLP expects a single-token input, but got shape {tuple(x.shape)}"
                )
            x = x[:, 0]
        elif x.dim() != 2:
            raise ValueError(
                f"TokenExpanderMLP expects [B, D] or [B, 1, D], but got shape {tuple(x.shape)}"
            )

        b, d = x.shape
        if d != self.dim:
            raise ValueError(
                f"TokenExpanderMLP dim mismatch: expected {self.dim}, got {d}"
            )
...
# =========================================================
# VisionTransformer：相对于原 CLIP 的关键修改
# =========================================================

class VisionTransformer(nn.Module):
    # [相对原始 CLIP 的修改]
    # 原始 CLIP 只输出 cls token 的全局视觉特征。
    # 这里改成同时输出：
    # 1) x_scene: scene token，全局场景特征
    # 2) x_visual: patch/object token，局部视觉特征
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = torch.cat(
            [self.class_embedding + torch.zeros(b, 1, x.shape[-1], device=x.device), x],
            dim=1
        )  # [B, HW+1, width]
        ...
        # cls token 作为全局场景特征
        x_scene = x[:, 0]  # [B, output_dim]

        # patch tokens 作为局部视觉特征
        x_visual = x[:, 1:] @ self.proj_visual  # [B, HW, output_dim]
...
class CLIPBackbone(nn.Module):
    ...
    在原始 CLIP 基础上做如下修改：
    1) image encoder 不只输出一个全局 image embedding，
       而是输出 scene_features 和 object_features
    2) text encoder 接收视觉上下文 context
    3) 最终输出：
       - TPrompt
       - VPrompt
    供下游 SAIST 使用
    ...
    def __init__(
        ...
        # =========================================================
        # Projection to prompt_dim
        # =========================================================
        self.projection = nn.Parameter(
            torch.empty(transformer_width, self.prompt_dim)
        )

        # =========================================================
        # SR-CLIP / SAIST 新增：上下文与视觉投影
        # =========================================================
        # learnable context 必须与 token_embedding / transformer 处于同一维度空间
        learned_ctx_len = max(1, min(16, self.context_length - 1))
        self.contexts = nn.Parameter(
            torch.randn(1, learned_ctx_len, transformer_width)
        )

        # scene token -> text transformer dim
        self.context_proj = nn.Sequential(
            nn.LayerNorm(embed_dim),
            nn.Linear(embed_dim, transformer_width),
        )

        # visual token -> prompt_dim
        self.visual_proj = nn.Sequential(
            nn.LayerNorm(embed_dim),
            nn.Linear(embed_dim, self.prompt_dim),
        )

        self.scene_prompt_proj = nn.Sequential(
            nn.LayerNorm(embed_dim),
            nn.Linear(embed_dim, self.prompt_dim),
        )
        ...
        self.scene_token_expander = TokenExpanderMLP(
            dim=self.prompt_dim,
            num_tokens=self.num_scene_prompt_tokens,
        )
        self.text_token_expander = TokenExpanderMLP(
            dim=self.prompt_dim,
            num_tokens=self.num_text_prompt_tokens,
        )

        self.object_object_refine = VisualToVisual(
            dim=self.prompt_dim,
            nhead=transformer_heads,
            dropout=0.1,
        )
        self.scene_object_refine = VisualToText(
            dim=self.prompt_dim,
            nhead=transformer_heads,
            dropout=0.1,
        )
    ...
    def encode_text(self, text, context):
        """
        text:    [K, L]
        context: [B, N_ctx, transformer_width]

        为了与 CLIP 的固定 context_length 兼容，
        这里不再把 context 插入成更长序列，
        而是将 BOS 后的一段 token 前缀替换为 scene + learnable context。
        """
        x_text = self.token_embedding(text)  # [K, L, C]
        k, n1, c = x_text.shape
        b, n2, c_ctx = context.shape

        if c_ctx != c:
            raise ValueError(
                f"context last dim ({c_ctx}) must match token embedding dim ({c})"
            )
        if n1 != self.context_length:
            raise ValueError(
                f"text sequence length ({n1}) must equal CLIP context_length ({self.context_length})"
            )
    ...
    def forward_features(self, image, text):
        ...
        输出：
          - tprompt: [B, K, prompt_dim]
          - vprompt: [B, N, prompt_dim]
        """
        ...
        object_tokens = self.visual_proj(object_features)  # [B, N, prompt_dim]
        object_tokens = self.object_object_refine(object_tokens)

        scene_prompt = self.scene_prompt_proj(scene_features)  # [B, 1, prompt_dim]
        scene_slots = self.scene_token_expander(scene_prompt)  # [B, M, prompt_dim]
        scene_slots = self.scene_object_refine(scene_slots, object_tokens)
        visual = torch.cat([scene_slots, object_tokens], dim=1)

        # scene/context -> transformer_width
        scene_context = self.context_proj(scene_features)  # [B, 1, transformer_width]
        context = torch.cat(
            [scene_context, self.contexts.expand(b, -1, -1)],
            dim=1,
        )
...
# =========================================================
# 对外封装：输出给 SAIST Mask Decoder 的 token
# =========================================================
...
class SRCLIP(nn.Module):
    ...
    输出：
      - text_token: [B, decoder_dim]
      - image_token: [B, decoder_dim]
      - TPrompt: [B, K, prompt_dim]
      - VPrompt: [B, N, prompt_dim]
    """
    def __init__(
        ...
        # [SR-CLIP / SAIST 新增]
        # 将高维 TPrompt 聚合后压缩到 decoder token 空间
        self.text_adapter = nn.Sequential(
            nn.LayerNorm(prompt_dim),
            nn.Linear(prompt_dim, prompt_dim),
            nn.GELU(),
            nn.Linear(prompt_dim, decoder_dim),
        )

        # [SR-CLIP / SAIST 新增]
        # 将高维 VPrompt 聚合后压缩到 decoder token 空间
    ...
    def forward(self, image, text):
        ...
        # 最简单的聚合方式：对 token 序列做平均池化
        text_prompt = tprompt.mean(dim=1)    # [B, prompt_dim]
        visual_prompt = vprompt.mean(dim=1)  # [B, prompt_dim]

        text_token = self.text_adapter(text_prompt)     # [B, decoder_dim]
        image_token = self.visual_adapter(visual_prompt)  # [B, decoder_dim]
...
def build_clip_backbone(state_dict: dict):
    ...
    构建改造后的 CLIPBackbone，而不是原始 CLIP 模型。
    兼容：
    1) 原始 CLIP checkpoint
    2) 修改后的 checkpoint（使用 projection 替代 text_projection）
    """
    ...
    # 原始 CLIP 使用 text_projection
    if "text_projection" in state_dict:
        embed_dim = state_dict["text_projection"].shape[1]
    else:
        # 修改版 checkpoint 使用 projection，输出 2 * embed_dim

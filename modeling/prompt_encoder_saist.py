    ...
    def __init__(
        ...
        if use_multiscale_context:
            self.local_branch1 = nn.Sequential(
                ...
                LayerNorm2d(out_chans) if use_layernorm else nn.Identity(),
                activation(),
                nn.Conv2d(
                    out_chans,
                    out_chans,
            ...
            self.local_branch3 = nn.Sequential(
                nn.Conv2d(out_chans, out_chans, kernel_size=5, padding=2, bias=False),
                LayerNorm2d(out_chans) if use_layernorm else nn.Identity(),
                activation(),
                nn.Conv2d(
                    out_chans,
            ...
            self.local_fuse = nn.Sequential(
                nn.Conv2d(out_chans * 3, out_chans, kernel_size=1, bias=False),
                LayerNorm2d(out_chans) if use_layernorm else nn.Identity(),
                activation(),
                nn.Conv2d(out_chans, out_chans, kernel_size=3, padding=1, bias=False),
                LayerNorm2d(out_chans) if use_layernorm else nn.Identity(),
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_multiscale_context:
            x = x + self.local_fuse(
                torch.cat(
                    [
                        self.local_branch1(x),
                        self.local_branch2(x),
                        self.local_branch3(x),
                    ],
                    dim=1,
                )
            )
